...
def make_work_order(bom_no, item, qty=0, project=None, variant_items=None, use_multi_level_bom=None):
	...
	if flt(qty) > 0:
		...
		bom_items = frappe.get_all(
			"BOM Item",
			filters={"parent": bom_no},
			fields=["item_code", "custom_length", "custom_width", "custom_thickness", "custom_density", "custom_kilogramskgs", "custom_raw_material_type", "custom_outer_diameter", "custom_inner_diameter", "custom_wall_thickness", "custom_base_weight"]
		)
		...
		for req_item in wo_doc.required_items:
			...
			if bi:
				...
				req_item.custom_raw_material_type = bi.custom_raw_material_type or 0
