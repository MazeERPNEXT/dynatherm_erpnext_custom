# import frappe

# @frappe.whitelist()
# def create_cutting_plan_from_bom(bom_name):
#     bom = frappe.get_doc("BOM", bom_name)

#     cp = frappe.new_doc("Cutting Plan")
#     cp.project = bom.project
#     cp.item_to_manufacture = bom.item

#     for row in bom.items:
#         cp.append("cutting_plan_item", {
#             "project": bom.project,
#             "item_code": row.item_code,
#             "uom": row.uom,
#             "qty": row.qty,
#             "length": row.custom_length,
#             "width": row.custom_width,
#             "thickness": row.custom_thickness,
#             "density": row.custom_density,
#             "kgs_per_unit": row.custom_kilogramskgs,
#             "total_weight":row.custom_total_weight,
#             "scrap_margin": row.custom_scrap_margin_percentage,
#             "scrap_margin_kg": row.custom_scrap_margin_kgs,
#         })

#     cp.insert(ignore_permissions=True)
#     return cp.name


import frappe

@frappe.whitelist()
def create_cutting_plan_from_bom(bom_name):
    bom = frappe.get_doc("BOM", bom_name)

    # -----------------------------
    # VALIDATIONS
    # -----------------------------
    if bom.docstatus != 1:
        frappe.throw("BOM must be Submitted before creating Cutting Plan")

    existing_cp = frappe.db.exists(
        "Cutting Plan",
        {"bom_no": bom.name, "docstatus": ["!=", 2]}
    )
    if existing_cp:
        frappe.throw(f"Cutting Plan already exists for BOM {bom.name}")

    # -----------------------------
    # CREATE CUTTING PLAN
    # -----------------------------
    cp = frappe.new_doc("Cutting Plan")
    cp.project = bom.project
    cp.bom_no = bom.name                # 🔑 IMPORTANT
    cp.item_to_manufacture = bom.item

    # -----------------------------
    # COPY BOM ITEMS → CUTTING PLAN ITEMS
    # -----------------------------
    for row in bom.items:

        # Filter only internal manufacturing items
        if not row.is_stock_item:
            continue
        if not row.include_item_in_manufacturing:
            continue
        if row.sourced_by_supplier:
            continue

        cp.append("cutting_plan_item", {
            "project": bom.project,
            "item_code": row.item_code,
            "uom": row.uom,
            "qty": row.qty,

            # Dimensions
            "length": row.custom_length,
            "width": row.custom_width,
            "thickness": row.custom_thickness,
            "density": row.custom_density,

            # Weight & Scrap
            "kgs_per_unit": row.custom_kilogramskgs,
            "total_weight": row.custom_total_weight,
            "scrap_margin": row.custom_scrap_margin_percentage,
            "scrap_margin_kg": row.custom_scrap_margin_kgs,

            # Optional traceability
            "bom_item_row_id": row.name
        })

    cp.insert(ignore_permissions=True)
    return cp.name

