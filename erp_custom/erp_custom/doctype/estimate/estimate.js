...
function fetch_bom_recursive(bom_name, opts, cb) {
	...
	if (visited.has(bom_name)) {
		return cb(null, []); // already processed
	}
	...
	get_items_from_bom(frm) {
		...
		fetch_bom_recursive(bom_name, { max_depth: 6 }, function (err, flat_items) {
			...
			flat_items.forEach(bom_item => {
				...
				// persist fields using frappe.model.set_value
				// use known fallback fields and custom_* fallbacks from your code
				try {
		...
		}); // end fetch_bom_recursive callback
	}, // end get_items_from_bom
